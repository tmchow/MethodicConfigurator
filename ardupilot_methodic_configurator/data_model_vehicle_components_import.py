...
class ComponentDataModelImport(ComponentDataModelBase):
    ...
    def _set_battery_type_from_fc_parameters(self, fc_parameters: dict[str, float]) -> None:  # pylint: disable=too-many-branches
        ...
        # if chemistry detection fails, use current chemistry if valid, otherwise default to BATTERY_DEFAULT_CHEMISTRY
        final_chemistry = detected_chemistry or current_chemistry or BATTERY_DEFAULT_CHEMISTRY
    ...
    def import_bat_voltage(self, specs: BatteryVoltageSpecs, param_name: str, voltage_type: str) -> None:
        if param_name in specs.fc_parameters:
            try:
                if voltage_type not in BATTERY_CELL_VOLTAGE_TYPES:
                    logging_error(_("Invalid voltage type %s for battery voltage import"), voltage_type)
                    return
                cell = round(specs.fc_parameters[param_name] / specs.estimated_cell_count, 4)
                if specs.limit_min <= cell <= specs.limit_max:
                    self.set_component_value(("Battery", "Specifications", voltage_type), cell)
                else:
                    logging_warning(
                        _("Calculated  %s voltage %.2f V is out of expected range for %s chemistry (%.2f - %.2f V)"),
                        voltage_type,
                        cell,
